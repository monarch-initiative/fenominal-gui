...
public class HpoMenuDownloader {
    ...
    public void downloadHpo(String dest) throws FenominalRunTimeException {
        try {
            URL url = new URL(HP_JSON_URL);
            logger.debug("Created url from "+HP_JSON_URL+": "+url);
            copyURLToFileThroughURL(url, new File(dest));
        } catch (MalformedURLException e) {
            logger.error(String.format("Malformed URL for %s [%s]",dest, HP_JSON_URL));
            logger.error(e.getMessage());
            throw new FenominalRunTimeException(e.getMessage());
        }
        System.out.println("[INFO] Downloaded " + dest);
    }
    ...
    private boolean copyURLToFileThroughURL(URL src, File dest)  {
        ...
        try {
            ...
            throw new FenominalRunTimeException("ERROR: Problem downloading file: " + e.getMessage());
        }
        return true;
    }
